...
def check_apartment_availability(request):
    ...
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            # agent = Agent.objects.get(id=agent_id, verified=True)
            ...
            if checkin_date > checkout_date:
                messages.error(request, "Check-in date must be before the Check-out date. Please select valid dates.")
                # stay on the same page – no redirect
                return render(request, 'core/apt_details.html', {'apartment': apartment})
            
            if checkin_date == checkout_date:
                messages.error(request, "Check-in date and Check-out date can not be the same.")
                # stay on the same page – no redirect
                return render(request, 'core/apt_details.html', {'apartment': apartment})
